...
data conversion overhead between Python and R. This is particularly
noticeable when:
...
Apache Arrow can significantly reduce these delays by optimizing data
transfer between Python and R. While there are no drawbacks to enabling
Arrow, the performance gains are negligible when processing small
amounts of data.
...
3.  Set the `use_arrow` argument to `TRUE` in the
    [`predict_species_from_audio_file()`](https://birdnet-team.github.io/birdnetR/reference/predict_species_from_audio_file.md)
    function

Credits to Danielle Navarro for the original solution[^1].

[^1]: Navarro, Danielle. 2022. “Passing Arrow Data Between R and Python
    with Reticulate.” September 9, 2022.
