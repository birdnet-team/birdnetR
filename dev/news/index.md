...
This is a **breaking release**. The package API has been redesigned to
align with the upstream `birdnet` Python package (\>=0.2.16,\<0.3).
Legacy entry points are removed without a deprecation cycle.
...
| `available_languages(version)` | [`supported_languages()`](https://birdnet-team.github.io/birdnetR/dev/reference/supported_languages.md) |
| `get_top_prediction()` | Removed; use `dplyr::slice_max()` or `birdnetTools` |

### Breaking changes

- All legacy model loaders (`birdnet_model_tflite()`,
  `birdnet_model_protobuf()`, `birdnet_model_meta()`,
  `birdnet_model_custom()`) are removed. Use
...
- `labels_path()` and `read_labels()` are removed. Use
  `get_species_list(model)` instead.
- `available_languages()` is removed. Use
...
- The `labels` argument in
  [`load_custom()`](https://birdnet-team.github.io/birdnetR/dev/reference/load_birdnet_model.md)
  is replaced by `species_list`.
- [`predict()`](https://rdrr.io/r/stats/predict.html) for geo models now
  defaults `min_confidence` to `0.03` to match the upstream Python
  `birdnet` default.
...
- [`load_custom()`](https://birdnet-team.github.io/birdnetR/dev/reference/load_birdnet_model.md)
  gains advanced arguments `classifier_type` and `is_raven` for custom
  model configurations.
- [`supported_model_configurations()`](https://birdnet-team.github.io/birdnetR/dev/reference/supported_model_configurations.md)
  returns all valid model configuration combinations.
- [`supported_languages()`](https://birdnet-team.github.io/birdnetR/dev/reference/supported_languages.md)
  returns the set of supported language codes.
- `get_species_list(model)` retrieves the species list from a loaded
  model.
- [`write_predictions()`](https://birdnet-team.github.io/birdnetR/dev/reference/write_predictions.md)
  saves prediction results directly from Python to CSV, Parquet, or
  NumPy format without crossing the R boundary.
- [`birdnet_version()`](https://birdnet-team.github.io/birdnetR/dev/reference/birdnet_version.md)
  returns the installed Python version, executable path, and `birdnet`
  package version.
- [`predict()`](https://rdrr.io/r/stats/predict.html) for acoustic
  models now exposes performance parameters `n_producers`, `n_workers`,
  `batch_size`, `prefetch_ratio`, `speed`, `half_precision`, and
  `max_audio_duration_min`; these default to `NULL` so the Python
  backend defaults are used unless explicitly set.
- [`predict()`](https://rdrr.io/r/stats/predict.html) for geo models now
  exposes `half_precision`, defaulting to `NULL` so the Python backend
  default is used unless explicitly set.
...
- [`supported_model_configurations()`](https://birdnet-team.github.io/birdnetR/dev/reference/supported_model_configurations.md)
  ...
  [`load_birdnet()`](https://birdnet-team.github.io/birdnetR/dev/reference/load_birdnet_model.md)
  ([\#45](https://github.com/birdnet-team/birdnetR/issues/45)).
- Integer-like parameter validation (`top_k`, `bandpass_fmin`, etc.) now
  silently accepts whole-number doubles (e.g. `5`) in addition to
  integer values (e.g. `5L`)
  ([\#45](https://github.com/birdnet-team/birdnetR/issues/45)).
- [`as.data.frame()`](https://rdrr.io/r/base/as.data.frame.html) for
  acoustic and geo prediction objects now converts results via a Python
  dict helper that calls `to_structured_array()` and returns plain
  Python lists and numpy arrays, bypassing `pandas` entirely. This fixes
  corrupt data frames caused by Arrow-backed `StringDtype` columns
...
- `available_languages()` **update**: A new argument has been added to
  `available_languages()` to specify the BirdNET version, making it more
  flexible for different model versions.
- **Renaming** `get_labels_path` to `labels_path()`. It now requires a
  model object as its first argument.
- `predict_species()` was **renamed** to
  `predict_species_from_audio_file()`
- `predict_species_at_location_and_time()` was **changed** to requirer a
  model object as first argument.
...
- **A new set of functions** (`birdnet_model_*`) to load pre-existing
  ...
  approach to model loading. See `?birdnet_model_load` for more details.
